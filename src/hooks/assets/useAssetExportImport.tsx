...
export interface UserMatchDetail {
  rawName: string;
  matchType: "exact" | "fuzzy" | "skipped" | null;
...
export interface DetailedImportResult {
  ...
  warnings: string[];
  userMatches: UserMatchDetail[];
  categoryIssues: CategoryMatchDetail[];
}
...
export const IMPORT_FIELD_GROUPS = {
  ...
  status: {
    ...
    fields: [
      ...
      { key: "event_notes", label: "Event Notes", required: false },
      { key: "asset_configuration", label: "Asset Configuration", required: false },
    ],
  },
} as const;
...
const mapStatus = (raw: string): string => {
  ...
  const map: Record<string, string> = {
    ...
    retired: "available", lost: "disposed", broken: "maintenance", stolen: "disposed", reserved: "available",
  };
...
export const classifyColumns = (headers: string[]): { mapped: string[]; unmapped: string[] } => {
  const importKeywords = [
    ...
    "site", "status", "vendor", "purchased from", "assigned to", "assigned_to",
    "event notes", "asset configuration",
  ];
  const mapped: string[] = [];
...
export function useAssetExportImport() {
  ...
  const importAssets = async (file: File) => {
    ...
    try {
      ...
      const [existingAssetsRes, usersRes] = await Promise.all([
        supabase.from("itam_assets").select("id, asset_tag, name, description, model, serial_number, status, purchase_price, purchase_date, warranty_expiry, notes, category_id, make_id, location_id, department_id, vendor_id, assigned_to, checked_out_to, asset_id").eq("is_active", true),
      ...
      (existingAssetsRes.data || []).forEach((asset: any) => { if (asset.asset_tag) existingMap.set(asset.asset_tag, asset as ExistingAsset); });

      /** Compare two field values, treating null/undefined/"" as equivalent */
      const fieldsEqual = (a: any, b: any): boolean => {
        if (a === b) return true;
      ...
      // Build user lookup maps for "Assigned to" resolution
      const allUsers = (usersRes.data || []) as { id: string; name: string; email: string }[];
      const usersByName = new Map<string, string>();
      const usersByEmail = new Map<string, string>();
      const userNameToFull = new Map<string, string>(); // lowercase name → original name
      allUsers.forEach((u) => {
        if (u.name) { usersByName.set(u.name.toLowerCase(), u.id); userNameToFull.set(u.name.toLowerCase(), u.name); }
        if (u.email) usersByEmail.set(u.email.toLowerCase(), u.id);
      });

      const SKIP_PATTERNS = ["stock", "gmbh", "pvt", "ltd", "retrieving data", "india pvt"];

      const resolveUserId = (raw: string): UserMatchDetail => {
        const result: UserMatchDetail = { rawName: raw || "", matchType: null, matchedName: null, matchedId: null };
        if (!raw) return result;
        const q = raw.trim();
        const ql = q.toLowerCase();

        // 1. Skip non-user values
        if (q.length > 50 || SKIP_PATTERNS.some((p) => ql.includes(p))) {
          return { ...result, rawName: q, matchType: "skipped" };
        }

        // 2. Exact name match
        if (usersByName.has(ql)) {
          return { rawName: q, matchType: "exact", matchedName: userNameToFull.get(ql)!, matchedId: usersByName.get(ql)! };
        }

        // 3. Exact email match
        if (usersByEmail.has(ql)) {
          const matched = allUsers.find((u) => u.email?.toLowerCase() === ql);
          return { rawName: q, matchType: "exact", matchedName: matched?.name || ql, matchedId: usersByEmail.get(ql)! };
        }

        const words = ql.split(/\s+/);

        // 4. First name + last initial (e.g. "Pranay M")
        if (words.length === 2 && words[1].length === 1) {
          ...
          const candidates = allUsers.filter((u) => {
            ...
            return parts[0] === firstName && parts.length >= 2 && parts[parts.length - 1].startsWith(initial);
          });
          if (candidates.length === 1) {
            return { rawName: q, matchType: "fuzzy", matchedName: candidates[0].name, matchedId: candidates[0].id };
          }
        }

        // 5. Email prefix match (derive firstname.lastname from input)
        if (words.length >= 2) {
          const derivedPrefixes = [
            `${words[0]}.${words[words.length - 1]}`, // first.last
            `${words[0]}.${words.slice(1).join("")}`,  // first.restjoined
            words.join(""),                              // alljoined
          ];
          for (const [email, id] of usersByEmail) {
            const emailPrefix = email.split("@")[0].toLowerCase();
            for (const dp of derivedPrefixes) {
              if (dp.length >= 4 && emailPrefix === dp) {
                const matched = allUsers.find((u) => u.id === id);
                return { rawName: q, matchType: "fuzzy", matchedName: matched?.name || email, matchedId: id };
              }
            }
          }
        }

        // 6. First + last word match (handles middle name variations like "Nayan Sanjay Patil" → "Nayan Patil")
        if (words.length >= 3) {
          const first = words[0];
        ...
        // 7. Compound join (e.g. "Rama Krishna" → "Ramakrishna")
        if (words.length >= 2) {
          const joined = words.join("");
          for (const [name, id] of usersByName) {
            const nameJoined = name.replace(/\s+/g, "");
            if (nameJoined === joined) {
              return { rawName: q, matchType: "fuzzy", matchedName: userNameToFull.get(name)!, matchedId: id };
            }
          }
        }

        // 8. Multi-word first name match (e.g. "Vishal Sujit Srivastav" → "Vishal Srivastav")
        if (words.length >= 2) {
          const first = words[0];
          const last = words[words.length - 1];
          const candidates = allUsers.filter((u) => {
            if (!u.name) return false;
            const parts = u.name.toLowerCase().split(/\s+/);
            return parts.length === 2 && parts[0] === first && parts[1] === last;
          });
          if (candidates.length === 1) {
            return { rawName: q, matchType: "fuzzy", matchedName: candidates[0].name, matchedId: candidates[0].id };
          }
      ...
      const allUserMatches: UserMatchDetail[] = [];
      const allCategoryIssues: CategoryMatchDetail[] = [];

      // Tag prefix → expected category mapping
      const TAG_PREFIX_CATEGORY: Record<string, string> = {
        "SVR": "Server", "LTP": "Laptop", "MTR": "Monitor",
        "EQP": "Equipment", "CLI": "Client",
      };

      /** Extract tag prefix from asset tag like "RT-SVR-01" → "SVR" */
      const extractTagPrefix = (assetTag: string): string | null => {
        const parts = assetTag.split("-");
        if (parts.length >= 2) {
          // Try each part against known prefixes
          for (const part of parts) {
            if (TAG_PREFIX_CATEGORY[part.toUpperCase()]) return part.toUpperCase();
          }
        }
        return null;
      };

      const insertBatch: Array<{ row: number; assetTag: string; data: any; fieldsSet: string[]; warnings: string[] }> = [];
      const updateBatch: Array<{ row: number; id: string; assetTag: string; data: any; fieldsSet: string[]; warnings: string[] }> = [];
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const categoryRaw = col(row, "Category", "category");
          const categoryId = await findOrCreate(lookups.categories, categoryRaw, "itam_categories");
          ...
          const description = col(row, "Description", "description") || col(row, "Asset Configuration") || null;
          const model = col(row, "Model", "model");
          const serialNumber = col(row, "Serial Number", "Serial No", "serial_number");
          const notes = col(row, "Notes", "notes") || col(row, "Event Notes") || null;
          const assignedToRaw = col(row, "Assigned to", "assigned_to");
          const userMatch = resolveUserId(assignedToRaw);
          const assignedToId = userMatch.matchedId;
          if (assignedToRaw && userMatch.rawName) allUserMatches.push(userMatch);

          // Track which fields have values for reporting
          const fieldsSet: string[] = [];
          const rowWarnings: string[] = [];
          if (name && name !== assetTag) fieldsSet.push("Name");
          if (description) fieldsSet.push("Description");
          if (model) fieldsSet.push("Model");
          if (serialNumber) fieldsSet.push("Serial No");
          if (purchasePrice !== null) fieldsSet.push("Cost");
          if (purchaseDate) fieldsSet.push("Purchase Date");
          if (warrantyExpiry) fieldsSet.push("Warranty Expiry");
          if (notes) fieldsSet.push("Notes");
          if (statusRaw) fieldsSet.push("Status");
          if (categoryId) fieldsSet.push("Category");
          if (makeId) fieldsSet.push("Brand");
          if (locationId) fieldsSet.push("Location");
          if (siteId) fieldsSet.push("Site");
          if (departmentId) fieldsSet.push("Department");
          if (vendorId) fieldsSet.push("Vendor");
          if (assignedToId) fieldsSet.push("Assigned to");
          // Category-tag prefix validation
          const tagPrefix = extractTagPrefix(assetTag);
          if (tagPrefix && categoryRaw) {
            const expectedCategory = TAG_PREFIX_CATEGORY[tagPrefix];
            if (expectedCategory && expectedCategory.toLowerCase() !== categoryRaw.toLowerCase()) {
              const issue: CategoryMatchDetail = { assetTag, tagPrefix, expectedCategory, actualCategory: categoryRaw };
              allCategoryIssues.push(issue);
              rowWarnings.push(`Tag prefix "${tagPrefix}" suggests "${expectedCategory}" but category is "${categoryRaw}"`);
            }
          }

          // Generate warnings based on match type
          if (assignedToRaw && userMatch.matchType === "fuzzy") {
            rowWarnings.push(`"${userMatch.rawName}" matched to "${userMatch.matchedName}" (approximate — verify)`);
          } else if (assignedToRaw && userMatch.matchType === "skipped") {
            rowWarnings.push(`"${userMatch.rawName}" appears to be a placeholder, skipped`);
          } else if (assignedToRaw && !assignedToId && userMatch.matchType === null) {
            rowWarnings.push(`User "${assignedToRaw}" not found in the system`);
          }
          ...
          if (assignedToId) {
            ...
            if (status === "in_use") {
              assetData.checked_out_to = assignedToId;
              if (!assetData.checked_out_at) assetData.checked_out_at = new Date().toISOString();
            }
          }
          ...
          const existing = existingMap.get(assetTag);
          if (existing) {
            // Diff: compare each field in assetData against existing record
            const changedFields: Record<string, any> = {};
            const changedFieldNames: string[] = [];
            
            const diffableFields: Array<{ dataKey: string; dbKey: keyof ExistingAsset; label: string }> = [
              { dataKey: "name", dbKey: "name", label: "Name" },
              { dataKey: "description", dbKey: "description", label: "Description" },
              { dataKey: "model", dbKey: "model", label: "Model" },
              { dataKey: "serial_number", dbKey: "serial_number", label: "Serial No" },
              { dataKey: "status", dbKey: "status", label: "Status" },
              { dataKey: "purchase_price", dbKey: "purchase_price", label: "Cost" },
              { dataKey: "purchase_date", dbKey: "purchase_date", label: "Purchase Date" },
              { dataKey: "warranty_expiry", dbKey: "warranty_expiry", label: "Warranty Expiry" },
              { dataKey: "notes", dbKey: "notes", label: "Notes" },
              { dataKey: "category_id", dbKey: "category_id", label: "Category" },
              { dataKey: "make_id", dbKey: "make_id", label: "Brand" },
              { dataKey: "location_id", dbKey: "location_id", label: "Location" },
              { dataKey: "department_id", dbKey: "department_id", label: "Department" },
              { dataKey: "vendor_id", dbKey: "vendor_id", label: "Vendor" },
              { dataKey: "assigned_to", dbKey: "assigned_to", label: "Assigned to" },
              { dataKey: "checked_out_to", dbKey: "checked_out_to", label: "Checked Out To" },
            ];

            for (const { dataKey, dbKey, label } of diffableFields) {
              if (dataKey in assetData && !fieldsEqual(assetData[dataKey], existing[dbKey])) {
                changedFields[dataKey] = assetData[dataKey];
                changedFieldNames.push(label);
              }
            }
            ...
            if (Object.keys(changedFields).length === 0) {
              ...
              skippedCount++;
              rowResults.push({ rowNum, assetTag, action: "skipped", fieldsUpdated: [], warnings: rowWarnings });
            } else {
              updateBatch.push({ row: rowNum, id: existing.id, assetTag, data: changedFields, fieldsSet: changedFieldNames, warnings: rowWarnings });
            }
          } else {
            insertBatch.push({ row: rowNum, assetTag, data: assetData, fieldsSet, warnings: rowWarnings });
          }
        } catch (rowError: any) {
      ...
      for (let i = 0; i < insertBatch.length; i += 50) {
        ...
        if (batchError) {
          for (const item of chunk) {
            ...
            if (singleError) {
              errors.push({ row: item.row, error: singleError.message });
              rowResults.push({ rowNum: item.row, assetTag: item.assetTag, action: "error", fieldsUpdated: [], warnings: item.warnings, error: singleError.message });
            } else {
              successCount += 1;
              createdCount += 1;
              rowResults.push({ rowNum: item.row, assetTag: item.assetTag, action: "created", fieldsUpdated: item.fieldsSet, warnings: item.warnings });
            }
          }
        } else {
          const count = inserted?.length || chunk.length;
          successCount += count;
          createdCount += count;
          for (const item of chunk) {
            rowResults.push({ rowNum: item.row, assetTag: item.assetTag, action: "created", fieldsUpdated: item.fieldsSet, warnings: item.warnings });
      ...
      for (let i = 0; i < updateBatch.length; i += 50) {
        ...
        results.forEach((res, idx) => {
          if (res.error) {
            errors.push({ row: chunk[idx].row, error: res.error.message });
            rowResults.push({ rowNum: chunk[idx].row, assetTag: chunk[idx].assetTag, action: "error", fieldsUpdated: [], warnings: chunk[idx].warnings, error: res.error.message });
          } else {
            successCount += 1;
            updatedCount += 1;
            rowResults.push({ rowNum: chunk[idx].row, assetTag: chunk[idx].assetTag, action: "updated", fieldsUpdated: chunk[idx].fieldsSet, warnings: chunk[idx].warnings });
          }
        });
        writesCompleted += chunk.length;
        setImportProgress({ current: writesCompleted, total: totalWrites });
      ...
      // Collect global warnings (unmatched users)
      ...
      if (successCount > 0) {
        ...
        toast.success(`Imported ${successCount} assets (${parts.join(", ")})`);
      }
      if (skippedCount > 0 && successCount === 0) {
        toast.info(`${skippedCount} rows skipped — all data identical`);
      }
      if (errors.length > 0) toast.warning(`${errors.length} rows had errors`);
      if (uniqueWarnings.length > 0) toast.info(`${uniqueWarnings.length} warnings`);

      return { success: successCount, created: createdCount, updated: updatedCount, skipped: skippedCount, errors, rows: rowResults, warnings: uniqueWarnings, userMatches: allUserMatches, categoryIssues: allCategoryIssues } as DetailedImportResult;
    } catch (error: any) {
      console.error("Import error:", error);
      toast.error("Failed to import assets: " + error.message);
      return { success: 0, created: 0, updated: 0, skipped: 0, errors: [], rows: [], warnings: [], userMatches: [], categoryIssues: [] } as DetailedImportResult;
    } finally {
