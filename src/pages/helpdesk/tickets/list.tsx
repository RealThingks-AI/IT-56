...
import { useUnifiedRequests } from "@/hooks/tickets/useUnifiedRequests";
import { isSLABreached } from "@/lib/tickets/ticketUtils";
...
export default function TicketsList() {
  ...
  const requests = useMemo(() => (allRequests || []).filter((request: any) => {
    ...
    if (filters.sla === 'breached' && !isSLABreached(request)) return false;
    if (filters.search) {
      const search = filters.search.toLowerCase();
      if (!(request.title?.toLowerCase().includes(search) || request.description?.toLowerCase().includes(search) || request.ticket_number?.toLowerCase().includes(search))) return false;
    }
  ...
  const getTicketColumnValue = useCallback((item: any, column: string): string | number => {
    switch (column) {
      case "ticket_number": return item.ticket_number || "";
      ...
      case "assignee": return getUserDisplayName(item.assignee) || "";
  ...
  const sortedRequests = useMemo(() => {
    if (!sortConfig.direction) return requests;
    return [...requests].sort((a, b) => {
      const aVal = getTicketColumnValue(a, sortConfig.column);
      const bVal = getTicketColumnValue(b, sortConfig.column);
      let cmp: number;
      if (typeof aVal === "number" && typeof bVal === "number") cmp = aVal - bVal;
      else cmp = String(aVal).localeCompare(String(bVal), undefined, { sensitivity: "base" });
      return sortConfig.direction === "desc" ? -cmp : cmp;
    });
  }, [requests, sortConfig, getTicketColumnValue]);
  ...
  const quickStatusChange = useMutation({
    mutationFn: async ({ ticketId, status }: { ticketId: number; status: string }) => {
      const { error } = await supabase.from("helpdesk_tickets").update({ status, updated_at: new Date().toISOString() }).eq("id", ticketId);
  ...
  const handleSelectAll = (checked: boolean) => {
    const pageTickets = sortedRequests.slice((currentPage - 1) * pageSize, currentPage * pageSize);
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      <TicketModuleTopBar exportData={requests} exportFilename="tickets-export">
        <div className="flex items-center gap-2 flex-1">
          <ToggleGroup type="single" value={filters.requestType || 'all'} onValueChange={(value) => { if (value) setFilters({ ...filters, requestType: value === 'all' ? null : value }); }} className="h-7">
            <ToggleGroupItem value="all" className="h-7 px-2.5 text-xs data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">All</ToggleGroupItem>
            <ToggleGroupItem value="ticket" className="h-7 px-2.5 text-xs data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">Tickets</ToggleGroupItem>
            <ToggleGroupItem value="service_request" className="h-7 px-2.5 text-xs data-[state=on]:bg-primary data-[state=on]:text-primary-foreground">Requests</ToggleGroupItem>
          </ToggleGroup>

          <div className="relative w-[180px]">
            <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
            <Input placeholder="Search..." value={filters.search || ''} onChange={e => setFilters({ ...filters, search: e.target.value })} className="pl-8 h-7 text-xs" />
          </div>
          ...
          {selectedIds.length >= 2 && (
            <Button variant="outline" size="sm" className="h-7 gap-1 text-xs" onClick={() => setMergeDialogOpen(true)}>
              <GitMerge className="h-3 w-3" /> Merge ({selectedIds.length})
            </Button>
          )}
        </div>
      </TicketModuleTopBar>
      ...
      <div className="flex-1 overflow-hidden">
        {isLoading ? (
          ...
          <TicketTableView
            ...
            onQuickStatusChange={(ticketId, status) => quickStatusChange.mutate({ ticketId, status })}
      ...
      {editTicket && (
        <EditTicketDialog ticket={editTicket} open={!!editTicket} onOpenChange={(open) => !open && setEditTicket(null)} />
      )}
      
      {assignTicket && (
        <AssignTicketDialog ticket={assignTicket} open={!!assignTicket} onOpenChange={(open) => !open && setAssignTicket(null)} />
      )}
