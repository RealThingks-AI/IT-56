-- Change entity_id from uuid to text to support both uuid and bigint source IDs
ALTER TABLE public.audit_logs 
  ALTER COLUMN entity_id TYPE text USING entity_id::text;

-- Update fn_audit_log to cast id to text explicitly
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (
      'Created', TG_TABLE_NAME, NEW.id::text, _app_user_id,
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    INSERT INTO public.audit_logs (action_type, entity_type, entity_id, user_id, metadata)
    VALUES (
      'Updated', TG_TABLE_NAME, NEW.id::text, _app_user_id,
      jsonb_build_object('old_values', to_jsonb(OLD), 'new_values', to_jsonb(NEW))
    );
  ELSIF TG_OP = 'DELETE' THEN
    INSERT INTO public.audit_logs (action_type, entity_type, entity_id, user_id, metadata)
    VALUES (
      'Deleted', TG_TABLE_NAME, OLD.id::text, _app_user_id,
